...
void mTube::doBanking(mLongPath *center) { 
	...
	for (i=0; i<MTUBE_CACHE_SIZE; ++i) { 
		...
		if (spanLen<0.0001f) { spanLen = 0.001f; }
		turnHint.scale(1.0f/spanLen); 
...
void mTube::mapCompletion(mVec3 *p, mVec3 *params, int *section) {
	...
	*section = next;
	...
	a.length3(&d); 
	if (d<0.00001f) { d = 0.0001f; }
	a.scale(1.0f/d);
	a.dotWith(&right, &ar); 
	theta = (real)acos(ar); 
	if (au<0.0f) {
		theta = (real)6.283185307179586476925286767 - theta;
	} 
	theta += rs.z; 
...
void mTube::getPointNormal(mVec3 *params, mVec3 *p, mVec3 *n, mVec3 *dir, mVec3 *right, mVec3 *centerp, mVec3 *shellp) {
	mVec3 rs, a, b, c, du, dr, up, tright, cdir, theta12; 
	int prev, next; 
	real localt, theta, disp, phiScale1, phiScale2, flatspot, clocalt; 

	getLocals(params->x, &localt, &prev, &next); 
	getBasis(prev, next, localt, centerp, dir, &up, &tright, &rs, &flatspot);
	right->copy(&tright);
	theta = params->y - rs.z; 
	disp = params->z;
	...
	b.addEq(&up); 
	tright.scale(rs.x * (real)cos(theta+0.1f) * phiScale2); 
	b.addEq(&tright); 
	sliderAdjuster(localt, prev, &b); 	

	// what an ass pain the third shellpt is!
	getLocals((params->x+0.01f), &clocalt, &prev, &next);  
	getBasis(prev, next, clocalt, centerp, &cdir, &up, &tright, &rs, &flatspot);
	theta = params->y - rs.z; 
	computeDent(flatspot, theta, &phiScale1); 
	c.copy(centerp); 
	up.scale((real)sin(theta) * phiScale1 * rs.y); 
	c.addEq(&up); 
	tright.scale((real)cos(theta) * phiScale1 * rs.x); 
	c.addEq(&tright); 
